...
`elsif SCL_sg13cmos5l_stdcell
    wire clk_b;
    wire gated_clk;

    // Lint for PDK cells expects power pins, so disable the warning
    /* verilator lint_off PINMISSING */
    sg13cmos5l_inv_1 CLKINV(.Y(clk_b), .A(clk));

    /* verilator lint_off GENUNNAMED */
    generate
        if (WIDTH <= 6)
            sg13cmos5l_lgcp_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
        else if (WIDTH <= 12)
            sg13cmos5l_lgcp_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
        else
            sg13cmos5l_lgcp_1 CG( .CLK(clk_b), .GCLK(gated_clk), .GATE(wen) );
    ...
    generate
        for (i = 0; i < WIDTH; i = i+1) begin : gen_latch
            sg13cmos5l_dlhq_1 state (.Q(data_out[i]), .D(data_in[i]), .GATE(gated_clk) );
        end
    endgenerate
    /* verilator lint_on PINMISSING */
`else
...
`elsif SCL_sg13cmos5l_stdcell
    ...
    generate
        if (WIDTH <= 6)
            sg13cmos5l_lgcp_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
        else if (WIDTH <= 12)
            sg13cmos5l_lgcp_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
        else
            sg13cmos5l_lgcp_1 CG( .CLK(clk), .GCLK(gated_clk), .GATE(wen) );
