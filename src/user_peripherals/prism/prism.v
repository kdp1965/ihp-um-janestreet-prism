...
`endif
   ...
   */
   ...
   =================================================================================
   */
   // The STEW field positions are not arithmetic any more: the decode below
   // is generated from the chroma compiler's config (chromas/tinyqv32.cfg,
   // `make stew-map` in chromas/), which lays the fields out by consumer so
   // each CFGMEM bank feeds one part of the machine.
   ...
   for (genvar f = 0; f <= FRACTURABLE; f++)
   begin: GEN_CTRL
      // ---- BEGIN STEW MAP (generated by chromas/stew_map.py from chromas/tinyqv32.cfg; edit the config, then `make stew-map`) ----
      // 128-bit STEW: 6 input muxes of 5 bits, 2 jump targets, 3 x 21 outputs, LUT tables 8, 8, 4, 4
      assign inc_si[f]           = stew[f][0];
      assign input_mux_sel[f][0] = stew[f][5:1];
      assign input_mux_sel[f][1] = stew[f][10:6];
      assign input_mux_sel[f][2] = stew[f][15:11];
      assign input_mux_sel[f][3] = stew[f][20:16];
      assign input_mux_sel[f][4] = stew[f][25:21];
      assign input_mux_sel[f][5] = stew[f][30:26];
      assign jump_to[f][0]       = stew[f][40:36];   // tree0 jump target
      assign jump_to[f][1]       = stew[f][35:31];   // tree1 jump target
      assign jump_outputs[f][0]  = {stew[f][125], stew[f][93], stew[f][122], stew[f][119], stew[f][116], stew[f][90], stew[f][87], stew[f][84], stew[f][81], stew[f][113], stew[f][110], stew[f][107], stew[f][104], stew[f][101], stew[f][98], stew[f][78], stew[f][61], stew[f][75], stew[f][72], stew[f][69], stew[f][66]};   // tree0 transition outputs, out 20..0
      assign jump_outputs[f][1]  = {stew[f][126], stew[f][94], stew[f][123], stew[f][120], stew[f][117], stew[f][91], stew[f][88], stew[f][85], stew[f][82], stew[f][114], stew[f][111], stew[f][108], stew[f][105], stew[f][102], stew[f][99], stew[f][79], stew[f][62], stew[f][76], stew[f][73], stew[f][70], stew[f][67]};   // tree1 transition outputs, out 20..0
      assign state_outputs[f]    = {stew[f][127], stew[f][95], stew[f][124], stew[f][121], stew[f][118], stew[f][92], stew[f][89], stew[f][86], stew[f][83], stew[f][115], stew[f][112], stew[f][109], stew[f][106], stew[f][103], stew[f][100], stew[f][80], stew[f][63], stew[f][77], stew[f][74], stew[f][71], stew[f][68]};   // default outputs, out 20..0
      assign cmp_sel[f][0]       = stew[f][48:41];   // tree0 LUT3
      assign cmp_sel[f][1]       = stew[f][56:49];   // tree1 LUT3
      assign cond_cfg[f][0]      = stew[f][60:57];   // cond0 LUT2
      assign cond_cfg[f][1]      = {stew[f][97], stew[f][96], stew[f][65], stew[f][64]};   // cond1 LUT2
      // ---- END STEW MAP ----
   end
